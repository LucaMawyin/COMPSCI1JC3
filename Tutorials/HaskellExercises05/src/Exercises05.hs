...
mergeSort xs
  | length xs == 1 = xs
  | length xs == 2 = merge a b
  | otherwise = merge (mergeSort a)  (mergeSort b)
  where 
    (a,b) = split xs
...
sortProp xs = 
  let
    sortProp' [x] [y] = x == y
    sortProp' (x:xs) (y:ys) = (x == y) && sortProp' xs ys

  in sortProp' xs (mergeSort xs)

  -- sortProp xs = xs == mergeSort xs
...
(!!) [] n = error "Empty List"
(!!) (x:xs) n
  | n >= length (x:xs) || n < 0 = error "Out of bounds"
  | n == 0 = x
  | otherwise = xs !! (n-1)
